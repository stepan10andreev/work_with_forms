...
export const PhoneInput: FC<IPhoneInput> = ({mask, heading, placeholderText, name}) => {
  ...
  const onBlur: ChangeEventHandler<HTMLInputElement> = (event) => {
    const currentTarget = event.target;
    // console.log(getOnlyPhoneNumber(currentTarget.value))
  ...
  return (
    <label className={styles.label}>
      ...
      <InputMask name={name} className={styles.input} mask={mask} onBlur={onBlur} onChange={onChange} value={getOnlyPhoneNumber(currentValue)} placeholder={placeholderText}/>
